...
def preprocess_data(df):
    ...
    text_features = ['RequestURIPath', 'RequestURIQuery', 'RequestBody', 'agent']
    ...
    preprocessor = ColumnTransformer(
        transformers=[
            ...
            ('text_user_agent', TfidfVectorizer(max_features=5000), 'UserAgent'),
...
def train_model(X_train, y_train, n_splits=5):
    """
        Trains an SVM model using Stratified K-Fold Cross-Validation. 
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(X_train, y_train)):
        ...
        fold_model =  SVC(kernel='linear', random_state=42, probability=True)
    ...
    print("\nTraining final SVC model on the full training dataset...")
    final_model =  SVC(kernel='linear', random_state=42, probability=True)
