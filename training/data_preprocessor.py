...
from config import (
    PERFORM_PREPROCESSING, TFIDF_MAX_FEATURES,
...
def preprocess_data(df, logger):
   ...
   # Ensure required columns exist, or provide default empty strings/values
    ...
    df['user_agent'] = df.get('UserAgent', '').fillna('') # Using 'UserAgent' from original log, ensure fillna too

    # Numerical features (e.g., lengths)
    df['request_length'] = df.get('RequestLength', 0).fillna(0) # Ensure numerical fillna
